...
- 🧪 **اختبارات آلية pytest** (192 اختبارًا) — `python -m pytest`
...
- 🔁 **CI** — `.github/workflows/ci.yml`: pytest + الفحص الموحد الحيّ (86) + فحص PWA (26) + حمل خفيف + بناء صورة Docker عند كل push/PR
...
python tests/live_container_test.py     # 132 فحصًا حيًا (SQLite)
...
│   ├── live_container_test.py # ✅ الفحص الحي (132 فحصًا داخل الحاوية)
...
### الاختبارات
```bash
python -m pytest                        # 192 اختبارًا آليًا
python tests/stack_check.py             # 86 فحصًا — BASE_URL / WAIT
python tests/live_container_test.py     # الفحص الحي داخل الحاوية (132 فحصًا — LIVE_BASE)
