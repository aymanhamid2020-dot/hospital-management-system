...
- 🧪 **اختبارات آلية pytest** (289 اختبارًا) — `python -m pytest`
...
### الاختبارات
```bash
python -m pytest                        # 289 اختبارًا آليًا
...
npm run test:e2e                     # 48 اختبارًا: الأطباء 7 + ملف الطبيب 2 + الأساسية 6 + المحاسبة 5 + شؤون الموظفين 3 + ملف المريض 2 + قائمة المرضى 6 + بوابة المريض 2 + إدارة المخازن 2 + اللقطات 4 + المعرض 9
