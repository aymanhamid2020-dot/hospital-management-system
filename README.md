...
- 🧪 **اختبارات آلية pytest** (121 اختبارًا) — `python -m pytest`
...
- 🧩 **تقسيم الواجهة** — `index.html` صار هيكلًا صغيرًا (89 سطرًا) + `app.js` (2328 سطرًا منطق SPA كامل) + `app.css` (132 سطرًا) — تُخدَّم بـ`no-cache` وتضمّنها قائمة شل PWA الخمسة
...
├── static/
│   ├── index.html           # 🖥️ هيكل الصفحة (89 سطرًا — عربي/إنجليزي)
│   ├── app.js               # 🖥️ منطق الواجهة SPA كاملًا (i18n + كل الشاشات — 2328 سطرًا)
...
### الاختبارات
```bash
python -m pytest                        # 121 اختبارًا آليًا
