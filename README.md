...
- 🧪 **اختبارات آلية pytest** (144 اختبارًا) — `python -m pytest`
...
- 🧩 **تقسيم الواجهة** — `index.html` صار هيكلًا صغيرًا (88 سطرًا) + `app.js` (2668 سطرًا منطق SPA كامل) + `app.css` (132 سطرًا) — تُخدَّم بـ`no-cache` وتضمّنها قائمة شل PWA الخمسة
...
│   ├── app.js               # 🖥️ منطق الواجهة SPA كاملًا (i18n + كل الشاشات — 2668 سطرًا)
...
│   ├── test_pharmacy.py     # 💊 تطوير الصيدلية: المنتهي/السلة/الإرجاع/الإتلاف/الوصفات/الإحصاءات (23)
...
│   ├── pharmacy_flow_check.py # 💊 تدفق تطوير الصيدلية حيًا (125 فحصًا — يحتاج خادمًا على 8001)
...
### الاختبارات
```bash
python -m pytest                        # 144 اختبارًا آليًا
...
python tests/pharmacy_flow_check.py     # 💊 تدفق الصيدلية (125 فحصًا — يحتاج خادمًا على 8001)
