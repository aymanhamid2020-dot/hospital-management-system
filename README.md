...
- 🗂️ **شاشة ملف المريض بالأقسام الستة داخل شاشة «المرضى»** — شريط تبويبات أعلى الشاشة («🧑‍🤝‍🧑 قائمة المرضى» ثم الأقسام الستة) **بلا نافذة منبثقة**: النقر على صف أو زر «🗂️ الملف» يفتح تبويب **الملف الشخصي** فورًا، وزر «قائمة المرضى» يعيد الجدول والفلاتر والصفحات (أوّل دخول للشاشة = القائمة) — وكل التبويبات تُغذّى من نقطة واحدة `GET /patients/{id}/chart`:
...
- 🗂️ **ملف الطبيب بالأقسام الخمسة** — زر «🗂️ الملف» في كل صف يفتح نافذة بخمسة تبويبات تُغذّى من طلب واحد `GET /doctors/{id}/chart`:
  1. **الملف الشخصي والمهني** — الاسم والتخصص العام والدقيق · **الدرجة العلمية** (استشاري/أخصائي/طبيب مقيم) · رقم الترخيص · الهاتف والبريد والعنوان و**الفرع** · **القسم** · **حساب المستخدم** المرتبط · **مدة الزيرة وسعر الكشفية وسعر الإعادة** (قابل للتحرير والحفظ)
  2. **المواعيد وجدول العمل** — نوبات العيادة الأسبوعية · **مناوبات الطوارئ/التنويم/الأونكول** · **الإجازات** · **أيام حظر الحجز** — و**الإجازة والحظر يمنعان حجز المواعيد فيها تلقائيًا** (حجزًا كان أم إعادة جدولة ⇒ 400 برسالة تذكر السبب)
  3. **الصلاحيات والتوقيع** — سبع صلاحيات تفصيلية (مرضاه فقط · مراد الطوارئ · طلب فحوصات/أشعة · وصف أدوية · فواتير · كشف حسابه) + **رفع التوقيع الإلكتروني والختم الطبي**
  4. **الحسابات والعمولات** — نسبة أو قيمة ثابتة لكل نوع خدمة + **كشف حساب** (إيراد ← مستحق ← محوَّل ← متبقي) + **تسجيل التحويلات** (يُرفض تجاوز الرصيد)
  5. **الأداء والإحصائيات** — **مرضى جدد/عائدون/طوارئ** · **نسبة الإلغاء ومتوسط الانتظار** · **أكثر الأدوية والفحوصات طلبًا** بأشرطة بيانية — مع اختيار الشهر
...
- 🔬 **المختبر والأشعة** — شاشة **بتبويبات**: `🧪 المختبر (LIS)` · `🩻 الأشعة (RIS)` · `📊 المشترك والتقارير`، ولكل قسم أقسامه الداخلية (LIS: طلبات · سحب وإدارة العينات · إدخال النتائج · اعتماد التقارير · دليل الفحوصات — RIS: طلبات · جدولة الأجهزة والغرف · صور الأشعة PACS · التقارير التشخيصية — المشترك: تسليم النتائج · المخزون · الإحصائيات). ويشمل: **سحب العيّنة مع توليد باركود** `BC{id:07d}` + استلام/رفض مسجّل السبب + **ملصقات باركود PDF** `/lab-orders/{id}/label?copies=1..20`، **دليل فحوصات** (سعر/صيام/أنبوب/عيّنة/وحدة/نطاق يُورَّث في كل طلب جديد)، **إدخال النتيجة** مع حساب «خارج النطاق/قيمة حرجة» تلقائيًا + إشعار، **الاعتماد والتوقيع الإلكتروني** (admin/doctor)، حالات (مسجّل → قيد التنفيذ → جاهزة → مراجَعة)، **ورقة النتيجة PDF** `/lab-orders/{id}/pdf?lang=ar|en` + **فلاتر الأولوية/حالة العيّنة/الشواذ** + **البحث** أعلى الجدول
...
- 🗂️ **شاشة المحاسبة بتبويباتها** — شاشة واحدة بخمسة تبويبات (📊 نظرة عامة · 🧾 المدينون · 💳 الفواتير · 📒 الدفتر العام · 📄 التقارير) تُبدَّل فورًا دون إعادة بناء الشاشة، و«🧾 الحسابات» و«💳 الفواتير» في القائمة اختصارات إلى تبويبيهما + **حارس ترتيب الرسم** (حاوية معزولة + رقم تسلسلي) يمنع كتابة تبويب متأخّر فوق محتوى الأحدث
- ⚡ **مركز العمليات السريعة** `/quick-ops` (شاشة `⚡ العمليات السريعة` في القائمة + اختصارات **Ctrl+Alt+S/B/T**) — الثلاث عمليات اليومية في **طلب واحد** بدل ثلاث شاشات:
  - **🛒 بيع سريع** `POST /quick-ops/sales`: سلة مختلطة من الأدوية والمستلزمات — الأدوية تُصرف كسجلات `Dispense` ⇒ **تظهر فورًا في «🛒 المبيعات» والصيدلية** وتُنقص رصيد الدواء بحركة `StockMovement` وتنبيه حد التنبيه، والمستلزمات ⇒ **فاتورة مريض** (خصم/ضريبة/دفع كامل أو جزئي) + **إذن صرف مرحّل** (FEFO)؛ المدفوع **يوزَّع على الأسطر بالتناسب** والقيد المزدوج يغطي العملية (نقدية/بنك عند السداد الكامل، ذمم مريض/تأمين عند الباقي).
  - **📥 شراء سريع** `POST /quick-ops/purchases`: مورد + أسطر (كمية/تكلفة/تشغيلة/انتهاء) ⇒ **إذن استلام مرحّل** + **فاتورة مورد** (رقمها يُولَّد تلقائيًا `V-00001` إن تُرك فارغًا) + **قيد ذمم دائنة** في معاملة واحدة.
  - **🔄 ترحيل سريع** `POST /quick-ops/transfers`: من مستودع إلى آخر بحركتي **خروج/دخول** في دفتر الحركات بعد التحقق من التوافر.
  - `GET /quick-ops/overview` (مؤشرات اليوم + الأكثر حركة + آخر العمليات) و`GET /quick-ops/catalog` (بحث لحظي بالاسم/الكود/الباركود مع المتاح والسعر المقترح)، وكل العمليات **ذرّية**: أي خطأ (كمية غير كافية، دواء منتهٍ، خصم أكبر من الإجمالي، مورد غير نشت) يُلغي العملية كاملة *(العمليات admin، والقراءة لأي مستخدم مسجّل)*.
...
- 🏢 **مركز الأقسام — ستة تبويبات** — شاشة «🏢 الأقسام» صارت مركزًا يغطي: **دليل الأقسام والهيكل** (شجرة أقسام رئيسية بوحدات فرعية + نوع كل قسم طبي/تشخيصي/إداري/خدمي + فعّال/معطّل + تكلفة تشغيله الشهرية) · **الأطباء والكادر** (الأطباء المنتسبون + **تعيين رئيس القسم HOD** + توزيع الكادر المساند كتمريض/تقني/إداري ومن هو رئيس القسم إداريًا) · **الغرف والأسرّة** (غرف بفئات: ملكي/خاصة/مشتركة/عناية مركزة/طوارئ/غرف عمليات + **توزيع كل سرير على غرفة** باحترام السعة + **حجز الغرف** برفض التداخل الزمني — والأسرة مدمجة هنا بدل شاشة منفصلة) · **الخدمات والأسعار** (كتالوج خدمات القسم بالسعر + **نسبة الطبيب** + **نسبة التأمين** محسوبةً لمبالغ، مع خطوات الإجراء) · **الجداول والمواعيد** (وردية العيادة: يوم + صباحي/مسائي + وقت الفتح والإغلاق، بلا تكرار) · **التقارير والإحصائيات** (نسبة الإشغال · الإيراد/المحصّل/التأمين مقابل المصروف الشهري والصافي · إنتاجية كل طبيب: مواعيد ومرضى وإيراد · لكل مريض) — والكتابة للمدير والقراءة للجميع
- ⬆️⬇️ **تصدير/استيراد CSV في كل القوائم** — محرّك واحد `/exchange` يخدم **الأصناف والموردين والأقسام والمستودعات والأدوية** (والمرضى والفواتير بمساراتها) بثلاثة أزرار في كل شريط أدوات: **تصدير CSV** (عربي بترميز Excel) · **📄 قالب** جاهز · **⬇️ استيراد CSV**. الاستيراد **دمج (upsert)** على مفتاح التفرّد: الموجود يُحدَّث والجديد يُضاف، **وكل صف معزول** — خطأ صف لا يوقف الدفعة ويُبلَّغ برقم سطره — مع قبول العناوين العربية والإنجليزية وترميز cp1256 القديم، وحماية (تصدير لكل مسجّل · استيراد للمدير فقط · 5MB و5000 صف)، والملف المُصدَّر يُستورد مجددًا بلا تكرار
...
- 🧪 **اختبارات آلية pytest** (303 اختبارًا) — `python -m pytest`
...
- 📦 **إدارة المخازن — الأقسام الستة** — شاشة «📦 المخزون» صارت بستة أقسام تعمل بطبقة `/stock`: **دليل المواد** بثلاثة تبويبات فرعية: 💊 الأدوية (مخزون الصيدلية: بطاقات القيمة + دفتر الحركات — وهو الافتتاحي) · 📋 قائمة المنتجات (اسم علمي/تجاري + باركود + شروط تخزين + حد أمان ونقطة إعادة طلب وحد أقصى) · 🔄 مستويات إعادة الطلب · **حركة المخازن** (مستودعات وفروع بأرصدة مستقلة · إذن استلام GRN برقم التشغيلة وتاريخ الانتهاء · تحويل بين المخازن · صرف لقسم/مريض · مرتجعات ومرتجع مورد) · **الجرد** (جلسة جرد فعلية تُغلق فتقارن العدّ بالرصيد وتقيّد فرقه كتسوية) · **الصلاحية** (دفعات/تشغيلات + صرف **FEFO** الأقرب انتهاءً أولًا + إهلاك موثّق) · **المشتريات** (طلب شراء PR · أمر شراء PO واستلامه يولّد GRN) · **التقارير** (بطاقة الصنف · تنبيهات انتهاء 30/60/90 · تقييم بسعر المتوسط وFIFO · الركود والأكثر حركة) — كل مستند يُنشئ حركات في `general_stock_movements` وأرصدة في `stock_balances` فلا رصيد بلا سند
...
- 👤 **بوابة المريض بحساب تمنحه الإدارة (لا تسجيل ذاتي)** — صفحة `/patient-portal.html` بتوكن منفصل `scope=patient_portal`، يرى فيها المريض **مواعيده وطلبات المختبر/الأشعة ووصفاته وفواتيره وطلبات رعايته** فقط. الحساب يُنشئه المدير من «🛡️ الجودة والموارد ← حسابات بوابة المريض» (اسم مستخدم + كلمة مرور 8 أحرف فأكثر + ربط بسجل مريض واحد) مع 🔑 إعادة تعيين كلمة المرور و⛔/✅ تنشيط وتعطيل الدخول وآخر دخول في الجدول، وتوكن البوابة مرفوض في مسارات الموظفين والعكس (فحص `scope` في `get_current_user`)
...
│   ├── patient-portal.html   # 👤 بوابة المريض (دخول باسم مستخدم يمنحه المدير + لوحة بيانات المريض)
...
│   │   ├── doctors-chart.spec.ts # 🩺 ملف الطبيب: التبويبات الخمسة + حفظ الملف المهني (2)
...
│   │   ├── portal.spec.ts     # 👤 2 اختباران: شرح إنشاء الحساب في صفحة الدخول + إنشاء حساب وتعطيله من الحوكمة (مع تنظيفه)
│   │   ├── inventory-ops.spec.ts # 📦 الأقسام الستة للمخزون + مكان كل تبويب + تدفّق استيراد/تصدير (4)
│   │   ├── departments.spec.ts # 🏢 تبويبات مركز الأقسام الستة + إضافة غرفة/خدمة/وردية (2)
...
│   ├── test_lab_lis.py      # 🧪 نواة LIS: دليل الفحوصات + دورة العيّنة + باركود/ملصقات + نتيجة/اعتماد/فلاتر (11)
│   ├── test_doctor_profile.py # 🩺 ملف الطبيب: الأقسام الخمسة · العمولات · كشف الحساب · منع الحجز (19)
...
│   ├── test_patient_portal_accounts.py # 👤 حسابات بوابة المريض: دورة كاملة + منع الموظف غير المدير (2)
│   ├── test_stock_ops.py     # 📦 إدارة المخازن: مستودعات + مستندات (استلام/تحويل/صرف/مرتجع/جرد) + FEFO + تقارير + حارس الحذف (11)
│   ├── test_exchange.py      # ⬆️⬇️ تبادل CSV: تصدير/قالب/دمج + صلاحيات + عزل أخطاء الصفوف (7)
│   ├── test_department_hub.py # 🏢 مركز الأقسام: الهيكل والكادر والغرف والخدمات والجداول والمؤشرات (7)
...
### الاختبارات
```bash
python -m pytest                        # 303 اختبارًا آليًا
python tests/stack_check.py             # 89 فحصًا — BASE_URL / WAIT
...
npm run test:e2e                     # 52 اختبارًا: الأطباء 7 + ملف الطبيب 2 + الأساسية 6 + المحاسبة 5 + شؤون الموظفين 3 + ملف المريض 2 + قائمة المرضى 6 + بوابة المريض 2 + إدارة المخزون 4 + مركز الأقسام 2 + اللقطات 4 + المعرض 9
...
- `tests/e2e/portal.spec.ts` — 👤 بوابة المريض: صفحة الدخول تشرح **أين يُنشأ الحساب** («يُنشئه موظف الاستقبال أو المدير» + المسار الفعلي «الجودة والموارد») والدخول الخاطئ يعرض رسالة عربية ولا يفتح اللوحة؛ ثم المدير ينشئ حسابًا باسم فريد (رفض كلمة المرور القصيرة قبل الإرسال، ثم صف يعرض «لم يدخل بعد» وزرّي كلمة المرور/تعطيل) ثم ⛔ تعطيله يقلب الحالة إلى «معطّل» — مع حذف الحساب التجريبي في النهاية فلا تترك الاختبارات بيانات في القاعدة.
...
- `GET /{id}/chart?month=YYYY-MM` — 🗂️ **ملف الطبيب الكامل في طلب واحد** يغذّي خمسة تبويبات: الملف المهني · المواعيد والجداول · الصلاحيات والتوقيع · الحسابات والعمولات · الأداء والإحصائيات *(كشف الحساب يظهر للمدير أو لمن فُعّلت له صلاحية `view_doctor_financials` فقط · `month` غير صالح ⇒ 400 · طبيب غير موجود ⇒ 404 · بلا توكن ⇒ 401)*
- `PUT /{id}/profile` — 💾 حفظ الملف المهني (الدرجة العلمية/التخصص الدقيق/الفرع/الهاتف/البريد/العنوان + أوقات الكشف + الصلاحيات) *(admin أو الطبيب نفسه؛ درجة خارج (استشاري/أخصائي/طبيب مقيم) أو مدة < 5 أو سعر سالب ⇒ 422 · بريد مكرر ⇒ 400 · حساب مستخدم غير موجود ⇒ 404)*
- `GET|POST|DELETE /{id}/shifts[/{sid}]` — 🚨 مناوبات الطوارئ/التنويم/الأونكول (نوع خارج الثلاثة ⇒ 422 · نهاية قبل بداية ⇒ 400 · حذف مكرّر ⇒ 404)
- `GET|POST|DELETE /{id}/leaves[/{lid}]` — 🌴 الإجازات (نهاية قبل بداية ⇒ 400)
- `GET|POST|DELETE /{id}/blocks[/{bid}]` — 🚫 أيام حظر الحجز (وقت بداية بلا نهاية أو العكس ⇒ 400 — اتركهما فارغين ليوم كامل)
- `GET|PUT /{id}/commissions` — 💰 بنود العمولة (نسبة أو قيمة ثابتة لكل نوع خدمة) *(الكتابة admin فقط؛ نسبة > 100 ⇒ 422 · نوع خدمة مكرر في الطلب ⇒ 400 — الاستبدال كامل)*
- `GET /{id}/ledger?month=` · `GET|POST /{id}/payouts` — 🧾 كشف الحساب (إيراد ← مستحق بالعمولة ← محوَّل ← متبقي) والتحويلات *(كشف الحساب والتحويلات للمدير أو صاحب الصلاحية فقط ⇒ 403 · تحويل يتجاوز الرصيد المتاح ⇒ 400 · `period` غير YYYY-MM ⇒ 400)*
- `POST|DELETE /{id}/signature` · `/{id}/stamp` · `GET /{id}/{kind}/image` — ✍️ التوقيع الإلكتروني والختم الطبي (صور png/jpg/jpeg/webp حتى 2 ميجابايت ⇒ صيغة أخرى 400 · كبير 413 · عرض صورة غير مرفوعة 404)
- 🖥️ الواجهة: بطاقة إحصاءات + بحث فوري + فلتر قسم/توافر + جدول بأعمدة **الدرجة** و**سعر الكشف** + زر **«🗂️ الملف»** لكل صف + ✏️ نافذة تعديل + ⛅ زر توافر سريع (للمدير أو الطبيب نفسه)
...
### المختبر والأشعة `/lab-orders`
- `GET /?patient_id=&doctor_id=&status=&test_type=&priority=routine|stat&sample_status=none|collected|received|rejected&abnormal=true|false` — الطلبات (الطبيب: طلبات مرضاه فقط) — **فلاتر جديدة**: الأولوية، حالة العيّنة، والشواذ؛ قيمة فلتر خاطئة ⇒ 400
- `POST /` — طلب تحليل/أشعة `{patient_id, doctor_id?, test_type: lab|radiology, test_name, price?, lab_test_id?, priority?, modality?, room?, scheduled_at?, specimen_type?, ref_min?, ref_max?, unit?}` *(200)* — اختيار `lab_test_id` يُورّث منه السعر وشرط الصيام ونوع الأنبوب/العيّنة والوحدة والنطاق الطبيعي
- `PUT /{id}` — تغيير الحالة أو إدخال `result` (**النتيجة مطلوبة قبل "جاهزة"** — عند الجاهزية يُنشأ إشعار `lab_result` وتُختم `result_at`)، ويقبل `report` (يُختم `reported_by/at`) و`modality/room/scheduled_at/priority`
- `POST /{id}/collect` — 🧫 سحب العيّنة `{specimen_type}` يولّد **باركود ثابتًا** `BC{id:07d}` ويحرّك الطلب إلى `in_progress` *(مسحوبة بالفعل ⇒ 409)*
- `POST /{id}/receive` — 📦 استلام `{accepted: true}` أو رفض `{accepted: false, reason}` — **الرفض يسجّل السبب في `notes`** *(ملغاة ⇒ 409)* — **قاعدة العمل: لا إدخال نتيجة لطلب `lab` قبل `sample_status = received`** *(400 «لا تُدخل النتيجة قبل سحب العيّنة واستلامها في المختبر»)*
- `POST /{id}/result` — 📥 إدخال النتيجة `{result, value?, unit?, ref_min?, ref_max?, critical?}` — يحسب **`abnormal`/`critical` تلقائيًا** (خارج النطاق = بعد الامتداد؛ حرجة = انحراف > 0.5×امتداد أو `value < 0.5×lo` أو `> 1.5×hi` لحدود موجبة) ويعيد الطلب إلى `ready` مع إشعار — **التعديل بعد الاعتماد ⇒ 400**
- `POST /{id}/verify` — ✍️ اعتماد `{note?}` *(admin/doctor فقط ⇒ غيرهما 403)* يوقّع إلكترونيًا (`verified_by/at`) ويحوّل الحالة إلى `reviewed`
- `GET /{id}/label?copies=1..20` — 🏷️ **ملصقات عيّنة باركود PDF** (شبكة A4 3×7 بـ Code39 مدمج) — نسخ خارج 1–20 ⇒ 422، وطلب بلا باركود ⇒ 400
...
### دليل الفحوصات `/lab-tests`
- `GET /?category=lab|radiology&active_only=true&q=…` — القائمة **لأي مستخدم مصادَق** (تصنيف خاطئ ⇒ 400) · `POST /` — إضافة `201` *(admin فقط — كود مكرر ⇒ 409 · `ref_min > ref_max` ⇒ 400)* · `PUT /{id}` — تعديل جزئي *(admin)* · `DELETE /{id}` — حذف *(admin)*
- حقول الفحص: `code, name, category, price, fasting_hours, tube_type, specimen_type, unit, ref_min, ref_max, active` — وهي مصدر الوراثة للطلبات ومنها تُبنى ملصقات العيّنة وعلامات النتيجة
...
### المخزون وإدارة المستودعات `/stock` *(القراءة لأي مستخدم مسجّل — الكتابة：admin)*
- `GET|POST /warehouses` · `PUT|DELETE /warehouses/{id}` (لا حذف لمستودع فيه أرصدة، والافتراضي واحد) · `GET /warehouses/{id}/items` — المستودعات وأرصدتها
- `GET|POST /docs` · `GET /docs/{id}` · `POST /docs/{id}/action` (`approve|cancel|complete`) · `POST /docs/{id}/receive` — مستندات موحّدة بأنواعها: `grn` استلام · `transfer` تحويل · `issue` صرف · `return` مرتجع · `supplier_return` مرتجع مورد · `stocktake` جرد · `pr` طلب شراء · `po` أمر شراء (استلامه يولّد `grn`)
- `GET /movements` — دفتر الحركات · `GET /items/{id}/card` — بطاقة الصنف (أرصدة + دفعات + حركات)
- `GET /reports/reorder` (حد الأمان/نقطة الطلب) · `reports/expiry?days=90` (دفعات + `days_left`) · `reports/valuation` (متوسط + FIFO) · `reports/slow-moving?days=90` (راكد/نشط) · `reports/summary`
- **قواعد ثابتة**: الصرف/التحويل يخصمان **FEFO** (الأقرب انتهاءً أولًا) ويرفضان تجاوز رصيد المستودع (400)، والاسم فريد (409)، والقراءة مفتوحة والكتابة للمدير فقط.

### مركز الأقسام `/department-hub` *(القراءة لكل مستخدم مسجّل — الكتابة：admin)*
- `GET /directory?include_inactive=` — دليل الأقسام كشجرة: الأقسام الرئيسية ووحداتها الفرعية، مع أرقامها (أطباء/أسرّة/غرف/خدمات/تكلفة شهرية)
- `PUT /{id}/structure` — تعديل الهيكل: `parent_id` (وحدة فرعية) · `dept_type` · `head_doctor_id` · `is_active` · `monthly_operating_cost` (نوع مجهول ⇒ 400 · قسم أب لنفسه أو دورة ⇒ 400)
- `GET /{id}/team` — الأطباء المنتسبون + الكادر المساند + رئيس القسم · `PUT /{id}/head` — تعيين/إلغاء رئيس القسم
- `POST|DELETE /{id}/support[/{row}]` — توزيع موظف على القسم (تمريض/تقني/إداري + رئيس إداري) — تكرار ⇒ 409
- `GET|POST|DELETE /{id}/rooms[/{room}]` — غرف القسم بفئاتها (ملكي/خاصة/مشتركة/عناية مركزة/طوارئ/غرف عمليات) وسعتها وأسرّتها
- `PUT /{id}/beds/{bed}/room?room_id=` — توزيع سرير على غرفة (السعة محترَمة ⇒ 400) · `GET /{id}/beds` — أسرّة القسم (مدمجة في شاشة الأقسام)
- `POST /{id}/rooms/{room}/bookings` · `GET /{id}/bookings` — حجز غرفة: **يرفض التداخل الزمني (409)** ونهايةً قبل بداية (400)
- `GET|POST|PUT|DELETE /{id}/services[/{service}]` — كتالوج الخدمات بالسعر ونسبة الطبيب ونسبة التأمين (مجموع النسب > 100% ⇒ 400) مع حساب `doctor_amount` و`insurance_amount` و`patient_amount`
- `GET|POST|DELETE /{id}/schedule[/{row}]` — وردية العيادة (يوم 0–6 · صباحية/مسائية · فتح<إغلاق؛ تكرار اليوم+الفترة ⇒ 409)
- `GET /analytics?days=` و`GET /{id}/analytics?days=` — الإشغال والإيراد/المصروف/الصافي وإنتاجية كل طبيب

### تبادل CSV `/exchange` *(تصدير لكل مسجّل · استيراد للمدير فقط)*
- `GET /resources` — الموارد المتاحة بأسمائها العربية وأعمدتها ومفتاح التفرّد (تغذّي أزرار الواجهة)
- `GET /{resource}/export.csv` — تصدير القائمة كاملة بترميز UTF-8+BOM (مورد مجهول ⇒ 404)
- `GET /{resource}/template.csv` — قالب بترويسة عربية + صف مثال صالح يُستورد مباشرة
- `POST /{resource}/import` — **دمج** ملف CSV (multipart) على مفتاح التفرّد: `created` و`updated` و`skipped` مع `errors` برقم السطر
- الموارد: `items` (أصناف المخزون العام بكل حقوله) · `vendors` · `departments` · `warehouses` · `medications`
- قواعد: تقبّل العناوين العربية والإنجليزية · ترميز `utf-8-sig` و`utf-8` و`cp1256` · كل صف معزول (خطأ صف لا يوقف الدفعة) · حدود 5MB و5000 صف · **الاستيراد لا يمسّ أرصدة المخزون** (الرصيد يُدار بإذن الاستلام) · الملف المُصدَّر يُستورد مجددًا بلا تكرار

### المخزون `/inventory` (الأدوية) + المخزون العام `/general-stock` *(القراءة لأي مستخدم مسجّل — التوريد/الجرد للمدير فقط)*
...
- `POST /login` — دخول باسم المستخدم وكلمة المرور ⇒ توكن `scope=patient_portal` (8 ساعات) وتسجيل `last_login_at` · `GET /me` — بيانات مريضه وحده (مواعيد، مختبر وأشعة، وصفات، فواتير، طلبات رعاية، تنويم) · `POST /appointments` — يحجز لموعده فقط
- `GET /accounts` · `GET /accounts/{id}` · `POST /accounts` · `PUT /accounts/{id}/password` · `PATCH /accounts/{id}` (تنشيط/تعطيل) · `DELETE /accounts/{id}` — **إدارة الحسابات (admin فقط)**: الاسم فريد على مستوى النظام، والحساب مربوط بسجل مريض واحد (فحص التكرار 400، والمريض غير موجود 404)، ولا تُعاد كلمة المرور في أي استجابة.
