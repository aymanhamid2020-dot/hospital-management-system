...
- 🧪 **اختبارات آلية pytest** (296 اختبارًا) — `python -m pytest`
...
│   │   ├── inventory-ops.spec.ts # 📦 الأقسام الستة للمخزون + تدفّق مستودع/صنف/إذن استلام (2)
...
### الاختبارات
```bash
python -m pytest                        # 296 اختبارًا آليًا
...
npm run test:e2e                     # 49 اختبارًا: الأطباء 7 + ملف الطبيب 2 + الأساسية 6 + المحاسبة 5 + شؤون الموظفين 3 + ملف المريض 2 + قائمة المرضى 6 + بوابة المريض 2 + إدارة المخازن 3 + اللقطات 4 + المعرض 9
