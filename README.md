...
- 🧪 **اختبارات آلية pytest** (191 اختبارًا) — `python -m pytest`
...
python tests/live_container_test.py     # 130 فحصًا حيًا (SQLite)
...
│   ├── live_container_test.py # ✅ الفحص الحي (130 فحصًا داخل الحاوية)
...
### الاختبارات
```bash
python -m pytest                        # 191 اختبارًا آليًا
python tests/stack_check.py             # 86 فحصًا — BASE_URL / WAIT
python tests/live_container_test.py     # الفحص الحي داخل الحاوية (130 فحصًا — LIVE_BASE)
