...
- 📱 **PWA** — `manifest.json` + أيقونة + عامل خدمة `sw.js` (قشرة التطبيق والطابور في الكاش — يعمل دون اتصال بعد أول زيارة) — **الشبكة أولًا** لملفات `/ui/` فلا يعرض المتصفح واجهة قديمة، + ترويسة `Cache-Control: no-cache` من الخادم — وفحص خاص `tests/pwa_offline_check.py` (25 فحصًا: التسجيل + أهداف الكاش المسبق + استراتيجية الطابور + صلاحية الـ manifest)
- 🖥️ **نسخة سطح مكتب** — `build_desktop.bat` تُنتج `dist\HospitalMS.exe` (خادم محلي + يفتح المتصفح + SQLite بجانب الملف)
- 🔥 **اختبار حمل وأداء** — `python tests/load_test.py` (متزامن × طلبات، زمن متوسط/p50/p95، حكم آلي PASS/FAIL) — ينجح حتى **50 متزامن × 50 = 2500 طلب بلا خطأ (p95 ≤ 800ms)**
...
- 🧩 **تقسيم الواجهة** — `index.html` صار هيكلًا صغيرًا (88 سطرًا) + `app.js` (2651 سطرًا منطق SPA كامل) + `app.css` (132 سطرًا) — تُخدَّم بـ`no-cache` وتضمّنها قائمة شل PWA الخمسة
...
- 🔁 **CI** — `.github/workflows/ci.yml`: pytest + الفحص الموحد الحيّ (86) + فحص PWA (25) + حمل خفيف + بناء صورة Docker عند كل push/PR
...
python tests/live_container_test.py     # 111 فحصًا حيًا (SQLite)
...
├── static/
│   ├── index.html           # 🖥️ هيكل الصفحة (88 سطرًا — عربي/إنجليزي)
│   ├── app.js               # 🖥️ منطق الواجهة SPA كاملًا (i18n + كل الشاشات — 2651 سطرًا)
│   ├── app.css              # 🖥️ أنماط الواجهة (132 سطرًا)
│   ├── monitor.html         # 📊 لوحة المراقبة المرئية
│   ├── manifest.json        # 📱 تعريف PWA (قابل للتثبيت)
│   ├── sw.js                # 📱 عامل الخدمة (شل v5 من 5 ملفات + الطابور دون اتصال)
...
│   ├── test_pharmacy.py     # 💊 تطوير الصيدلية: المنتهي/السلة/الإرجاع/الإتلاف/الوصفات/الإحصاءات (21)
│   ├── test_db_resilience.py # 🛡️ صلابة القاعدة: WAL + توازن الاتصال + الفهارس + اتساق النسخ تحت WAL (5)
│   ├── stack_check.py       # ✅ الفحص الموحد (86 فحصًا — قراءة فقط)
│   ├── live_container_test.py # ✅ الفحص الحي (111 فحصًا داخل الحاوية)
...
│   ├── pharmacy_flow_check.py # 💊 تدفق تطوير الصيدلية حيًا (116 فحصًا — يحتاج خادمًا على 8001)
│   ├── pwa_offline_check.py # 📱 فحص PWA/العمل دون اتصال (25 فحصًا — يحتاج خادمًا على 8001)
...
- `GET /reports/pharmacy/csv?section=inventory|dispenses|disposals` — ⬇️ CSV المخزون أو سجل الصرف (بحقول الجرعة وحالة الدفع والمرتجع) أو حركات الإتلاف/الإرجاع *(admin — قسم خاطئ ⇒ 400)*
...
### الاختبارات
```bash
python -m pytest                        # 142 اختبارًا آليًا
python tests/stack_check.py             # 86 فحصًا — BASE_URL / WAIT
python tests/live_container_test.py     # الفحص الحي داخل الحاوية (111 فحصًا — LIVE_BASE)
...
python tests/pharmacy_flow_check.py     # 💊 تدفق الصيدلية (116 فحصًا — يحتاج خادمًا على 8001)
python tests/pwa_offline_check.py       # 📱 PWA + العمل دون اتصال (25 فحصًا — يحتاج خادمًا على 8001)
...
USERS=50 REQUESTS=50 python tests/load_test.py    # حمل أثقل: 2500 طلب — p95 ≤ 800ms
```
