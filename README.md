...
- 🧪 **اختبارات آلية pytest** (180 اختبارًا) — `python -m pytest`
...
python tests/live_container_test.py     # 117 فحصًا حيًا (SQLite)
...
│   ├── live_container_test.py # ✅ الفحص الحي (117 فحصًا داخل الحاوية)
...
### الاختبارات
```bash
python -m pytest                        # 180 اختبارًا آليًا
python tests/stack_check.py             # 86 فحصًا — BASE_URL / WAIT
python tests/live_container_test.py     # الفحص الحي داخل الحاوية (117 فحصًا — LIVE_BASE)
