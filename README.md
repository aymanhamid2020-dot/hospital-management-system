...
- 🗂️ **شاشة المحاسبة بتبويباتها** — شاشة واحدة بسبعة تبويبات (📊 نظرة عامة · 🛒 المبيعات · 🧾 المدينون · 🧾 الفواتير · 📒 الدفتر العام · 📄 التقارير · 💵 الرواتب) تُبدَّل فورًا دون إعادة بناء الشاشة، وروابط القائمة الجانبية (المبيعات/الحسابات/الرواتب/الفواتير) اختصارات إلى تبويبها + **حارس ترتيب الرسم** (حاوية معزولة + رقم تسلسلي) يمنع كتابة تبويب متأخّر فوق محتوى الأحدث
...
- 🧪 **اختبارات آلية pytest** (204 اختبارات) — `python -m pytest`
...
- 🧩 **تقسيم الواجهة** — `index.html` صار هيكلًا صغيرًا (111 سطرًا) + `app.js` (3696 سطرًا منطق SPA كامل) + `app.css` (284 سطرًا) — تُخدَّم بـ`no-cache` وتضمّنها قائمة شل PWA الخمسة
...
- 🔁 **CI** — `.github/workflows/ci.yml`: pytest + الفحص الموحد الحيّ (86) + فحص PWA (26) + حمل خفيف + **مصفوفة E2E (3 متصفحات × بايثون 3.12/3.13 = 6 وظائف)** + **بوابة انسداد توليد `docs/user-guide.pdf`** + بناء صورة Docker عند كل push/PR
...
├── static/
│   ├── index.html           # 🖥️ هيكل الصفحة (111 سطرًا — عربي/إنجليزي)
│   ├── app.js               # 🖥️ منطق الواجهة SPA كاملًا (i18n + كل الشاشات — 3696 سطرًا)
│   ├── app.css              # 🖥️ أنماط الواجهة (284 سطرًا)
...
### الاختبارات
```bash
python -m pytest                        # 204 اختبارًا آليًا
...
npm run test:e2e                     # 30 اختبارًا: الأطباء 6 + الأساسية 6 + المحاسبة 5 + اللقطات 4 + المعرض 9
